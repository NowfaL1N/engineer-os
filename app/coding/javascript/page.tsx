/**
 * JavaScript Language Overview Page
 * 
 * Displays all JavaScript topics organized by categories.
 * Route: /coding/javascript
 */

import Link from "next/link"
import { getLanguageById } from "@/services/coding.service"
import { getJavaScriptTopicsByCategory } from "@/services/coding/javascript-data-adapter"
import { Button } from "@/components/ui/button"

export const metadata = {
  title: "JavaScript Programming - Coding Practice",
  description: "Learn JavaScript programming with comprehensive tutorials and practice exercises",
}

export default function JavaScriptLanguagePage() {
  const language = getLanguageById("javascript")
  const categories = getJavaScriptTopicsByCategory()
          ...
          <section key={category.id} className="border rounded-lg p-6 bg-card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {category.topics.map((topic) => (
                <Link
                  key={topic.id}
                  href={`/coding/javascript/${topic.id}`}
                  className="group"
                >
